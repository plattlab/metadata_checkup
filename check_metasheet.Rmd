...
check_metadata <- function(metadata, allowed_indices) {
  metadata_subset <-
    metadata %>%
    dplyr::select(Sample, EXTERNAL_SAMPLE_NAME, BARCODES, INDEX2) %>%
    set_names(c("sample_num", "sample_name", "idx1", "idx2")) %>%
    mutate(
...
#metadata_filepath <- file.path(files_dir, "20230324_Metadata_NextSeq_SR150_HIGH_OB.xlsx")
